...
43. `CITY_CODES_MAP`는 `"SEL": "SEL"`을 포함해야 하며, `SEL -> CJU`는 인터파크 URL에서 `c:SEL-c:CJU` 형태로 생성되어야 한다.
44. 공항 콤보 옵션은 `ui.airport_options` 공용 helper를 사용한다. 국내선 모드는 `config.DOMESTIC_AIRPORT_CODES`만 노출하고, 국제선 모드는 기본 공항 + custom preset을 허용한다.
45. 다중 목적지/날짜 범위/가격 알림 다이얼로그는 국내선/국제선 라디오를 제공하되 기존 `search_requested` signal signature를 유지한다.
46. CSV/Excel export는 `ui.export_helpers` 공용 helper를 사용한다. 공통 컬럼에는 `return_airline`, `benefit_price`, `benefit_label`, `outbound_price`, `return_price`가 포함되어야 한다.
47. `AlertAutoCheckWorker`는 `alert_no_result(int, str, str)` signal을 제공한다. 0건 결과는 `last_error = "NO_RESULT: 검색 결과 없음"`으로 저장하고 알림 목록에는 `결과 없음`으로 표시한다.
48. 국내선 왕복 조합 dedup key는 출발/도착 시간, 편명, API key, 혜택가/혜택 라벨을 포함해 서로 다른 실제 항공편을 보존한다.
49. `PreferenceManager`는 `advanced_search_history`와 `add_advanced_history()`/`get_advanced_history()`를 제공하며, 고급 검색 요약은 최대 20개까지만 저장한다.
...
67. 구버전 `favorites` 테이블에 `dedup_key`가 없을 수 있으므로 `idx_fav_dedup_key` 인덱스는 `_migrate_schema_if_needed()`가 컬럼을 추가한 뒤 생성해야 한다.
68. PyInstaller spec 3종은 `storage.schema`, `storage.flight_database`, `storage.db_favorites` hiddenimport를 유지하며, 이번 DB 마이그레이션 수정에는 spec 변경이 필요하지 않다.
69. `.gitignore`는 `.db`/`.db-*`뿐 아니라 `*.sqlite`, `*.sqlite3`, `*.sqlite-journal`, `*.sqlite3-journal`도 제외해야 한다.
70. 2026-05-11 DB 마이그레이션 후속 검증 기준선은 `pytest -q -> 98 passed`, `pyright --warnings -> 0 errors`, `check_tracked_text.py --check-lf -> Checked 112 tracked text files: OK`다.
