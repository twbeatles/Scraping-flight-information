...
1. `gui_v2.py`? ??? facade?? ?? ??? `app/main_window.py`, `app/mainwindow/*`? ???????.
2. `scraper_v2.py`? facade?? ?? ??? `scraping/*`? ???????.
3. `database.py`? facade?? ?? ??? `storage/*`? ???????.
4. `ui/components.py`, `ui/dialogs.py`, `ui/workers.py`? facade?? ?? ??? `ui/*_*.py`? ???????.
5. ??/???? ?????.
   - `python gui_v2.py`
