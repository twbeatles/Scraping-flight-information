...
현재 코드베이스는 2026-05-11 API-first 복구와 DB 마이그레이션 후속 조치를 이미 반영한 상태다. 이전 감사의 상세 실패 항목은 운영 기준에서 제거하고, 아래 현재 구조와 검증 기준을 최신 기준으로 둔다.
...
new package text check
-> Checked 30 new package files: OK
...
- 실사이트 API 계약은 외부 서비스 변경 영향을 받으므로 릴리스 전에는 필요 시 `python scripts/live_smoke_search.py`로 별도 확인한다.
